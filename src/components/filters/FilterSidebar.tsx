          ...
          {ratings && onRatingChange && selectedRating !== undefined && (
            <div className="flex items-center space-x-2 flex-1 min-w-[140px]">
              ...
              <select
                ...
                className="flex-1 px-3 sm:px-4 py-2 sm:py-2.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg sm:rounded-xl text-gray-700 dark:text-gray-300 font-medium text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-[#1B6EF3] focus:border-transparent transition-all duration-200"
              >
                {ratings.map((rating) => (
        ...
        <button
          ...
          className={`flex-shrink-0 px-4 sm:px-6 lg:px-8 py-2 sm:py-2.5 lg:py-3 rounded-full font-medium text-sm sm:text-base transition-all duration-300 border whitespace-nowrap cursor-pointer ${
            activeFilter === filter.value
              ? 'bg-gradient-to-r from-[#1B6EF3] to-[#1B6EF3] text-white border-transparent shadow-lg'
              : 'bg-white text-gray-700 border-gray-300 hover:border-[#1B6EF3] hover:bg-[#1B6EF3]/5'
          }`}
        >
          {filter.label}
        </button>
