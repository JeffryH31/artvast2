...
const FeaturedDesignersSection: React.FC = () => {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900 dark:text-white mb-4 transition-colors duration-300">
              {t.featuredDesigners.featured} <span className="bg-gradient-to-r from-[#1B6EF3] to-[#1B6EF3] bg-clip-text text-transparent">{t.featuredDesigners.designers}</span>
            </h2>
  ...
  return (
    <section className="py-16 sm:py-20 lg:py-24 bg-gradient-to-br from-gray-50 via-white to-gray-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 relative overflow-hidden transition-colors duration-300">
      ...
      <div className="absolute top-20 right-20 w-80 h-80 bg-[#1B6EF3]/10 dark:bg-[#1B6EF3]/20 rounded-full blur-3xl"></div>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
          {featuredDesigners.map((designer) => (
            <Link
              ...
              className="group bg-white/5 backdrop-blur-sm rounded-2xl p-6 shadow-lg hover:shadow-[0_8px_32px_rgba(27,110,243,0.2)] transform hover:scale-105 transition-all duration-500 border border-white/10 hover:border-[#1B6EF3]/30">
              {/* Avatar */}
              <div className="relative mb-4">
                <div className={`w-20 h-20 sm:w-24 sm:h-24 bg-gradient-to-br ${designer.avatar_gradient || 'from-[#DFE7F7] to-[#1B6EF3]'} rounded-full mx-auto flex items-center justify-center text-white text-2xl sm:text-3xl font-bold shadow-lg group-hover:scale-110 transition-transform duration-300`}>
                  {designer.avatar_initials}
                </div>
                {designer.verified && (
                  <div className="absolute bottom-0 right-1/2 translate-x-8 w-6 h-6 bg-gradient-to-r from-[#1B6EF3] to-[#1755E0] rounded-full flex items-center justify-center shadow-lg">
              ...
              <div className="text-center">
                <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-1 group-hover:text-[#1B6EF3] transition-colors">
                  {designer.name}
                </h3>
                ...
                <div className="flex flex-wrap justify-center gap-1">
                  {designer.specialties?.slice(0, 2).map((specialty) => (
                    <span
                      key={specialty}
                      className="px-2 py-0.5 bg-[#1B6EF3]/10 text-[#1B6EF3] text-xs font-medium rounded-full"
                    >
                      {specialty}
                    </span>
                  ))}
        ...
        <div className="text-center mt-10 sm:mt-12">
          <Link
            href="/designers"
            className="inline-flex items-center gap-2 px-8 py-3 bg-gradient-to-r from-[#1B6EF3] to-[#1B6EF3] text-white font-semibold rounded-xl hover:shadow-xl transform hover:scale-105 transition-all duration-300"
          >
